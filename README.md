...
# 🚀 Hunter_V2: 混合架構量化交易系統 (XAUUSD)
# 最新版本設為private

> **專案狀態**: **[私有專案 PRIVATE]** > 本儲存庫包含核心交易邏輯與實盤風控演算法。以下內容旨在說明其系統架構、技術迭代細節及量化指標。

## 📌 專案概述
Hunter_V2 是從 V1 研究專案演進而來的專業版本。其核心設計理念在於將 **訊號產生 (Python)** 與 **執行監控風控 (C++)** 進行解耦，建構出一個能在 MetaTrader 5 (MT5) 環境下穩定運行的自動化交易系統。

## 🛠 混合架構設計
為了達到機構級的穩定性，系統分為以下兩層運行：

### 1. 策略決策層 (Python)
* **核心模型**：隨機森林分類器 (Random Forest Classifier)。
* **特徵工程**：整合 `bb_width` (布林帶寬)、`rsi`、`ma_dist` (均線偏離) 及市場時段等 10 項多維特徵。
* **資金管理**：導入動態 **半凱利準則 (Half-Kelly Criterion)**，在資產成長與波動率之間取得量化平衡。

### 2. 執行監控層 (C++ / MT5 API)
* **硬性風控引擎**：使用 C++ 開發高效能監控模組，與 MT5 終端進行毫秒級通訊。
* **強制限制**：由底層硬性鎖定單筆手數為 0.02 lot，並即時監控每日回撤上限，攔截任何違反風控參數的下單請求。
* **執行品質管理**：即時偵測市場點差 (Spread) 與滑點，確保策略僅在流動性充足時入場，降低隱形成本。

## 📊 績效摘要 (基於 Hunter_V1 框架)
在 8R 高盈虧比設定下，展現出強大的 Alpha 捕捉能力：
* **利潤因子 (Profit
